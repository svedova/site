...
export default () => (
  <Post>
    ...
    <P>
      Da ich es an meinem {`"`}Arbeitsplatz{`"`} ja gerne
      ...
      auf deinem Win7-Desktop entfernen kannst.
    </P>

    <P>
      Gleich zu Anfang: Falls du schlicht keine Lust auf die unten
      stehende Anleitung hast, kannst du dir auch gerne
      ein
      {' '}
      <Link href="http://www.delightsoftware.com/de/ShortcutOverlayRemover">
        Programm
      </Link>
      {' '}
      laden, welches
      die Arbeit dann für dich erledigt.
    </P>

    <P>
      Da ich selbst grundsätzlich allerdings nicht so
      ...
      in eine kurze Anleitung zusammen:
    </P>
    ...
    <OL>
      ...
      <LI>
        Nachdem du die Datei geöffnet hast, navigiere zu
        ...
        entstandene Ordner muss dann {`"`}Shell Icons{`"`} genannt werden.)
      </LI>

      <LI>
        Nun musst du im {`"`}Shell Icons{`"`}-Verzeichnis
        ...
        die Zahl <InlineCode>29</InlineCode> ein.
      </LI>

      <LI>
        Als nächstes klickst du doppelt auf die
        ...
        ein leeres Icon):
        {' '}
        <InlineCode>C:Windowssystem32imageres.dll,-121</InlineCode>
      </LI>

      <LI>
        <b>Achtung:</b> (Falls im {`"`}Shell Icons{`"`}-Ordner bereits
      ...
      <LI>
        Hast du den Pfad angegeben, kannst du ihn mit einem
        Klick auf {`"`}OK{`"`} abspeichern. Nun musst du dein System nur
        noch einmal neu starten, und schwups: Der nervige Pfeil ist weg! :)
      </LI>
    </OL>
    ...
    <P>
      Du möchtest dein Verknüpfungs-Pfeil wiederherstellen? Kein
      Problem! Lösche einfach den Ordner {`"`}Shell Icons{`"`}, welchen
      du nach der obigen Anleitung erstellt hast, und starte deinen
      PC nochmals neu. Schon ist er wieder da!
    </P>

    <P>
      Hast du zu diesem Thema noch irgendwelche Fragen, oder
