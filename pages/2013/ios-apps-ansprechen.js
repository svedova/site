...
export default () => (
  <Post>
    ...
    <P>
      Wie einige von euch sicher schon gemerkt haben, sind die
      generierten Buttons zum Twittern und Flattrn meiner Artikel
      vom unteren Teil der Beiträge verschwunden.
    </P>

    <P>
      Weshalb? Zum einen verlängern die geladenen Scripte die Ladezeit
      um einige wertvolle Millisekunden - ein viel wichtiger Grund
      zur Abschaffung war jedoch folgender: Ich möchte direkt
      installierte iOS-Apps ansprechen, falls diese vorhanden sind.
    </P>

    <P>
      Also habe ich mich auf die Suche gemacht und bin auf
      ...
      der offiziellen Twitter-App gegriffen.
    </P>

    <P>
      Im Grunde lassen sich die URLs nach dem modifizieren mit eigenen
      Inhalten ganz einfach durch Javascript oder HTML-Links
      absprechen. Jedoch tut sich nun ja folgendes Problem auf: Was wenn
      der Nutzer über seinen Desktop-PC / Mac zugreift?
    </P>

    <P>
      Hierfür währe das URL-Schema ja nutzlos, da keine Twitter-App
      ...
      Twitter Webclients aufrufen.
    </P>

    <P>
      Vom Browser aus zu prüfen, ob eine App auf dem eigenen Gerät
    ...
    <P>
      ...falls diese nicht erreicht werden kann, soll der Webclient aufgerufen
  ...
  window.location = "https://twitter.com/intent/tweet?source=webclient&amp;text=zu teilender Text";
    ...
    <P>
      Das ganze packen wir dann am besten noch in eine eigene
      Funktion, welche wir mit Hilfe des
      {' '}
      {`"`}
      OnClick
      {`"`}
      -Eventhandlers z.B. durch
      einen HTML-Link ansprechen. Nun versucht der Browser die
      iOS-App zu öffnen - wenn diese nicht erreicht werden kann, wird
      der Link des Webclients gecallt.
    </P>

    <P>
      Solange sie URL-Schemes bieten klappt das Ganze auch mit allen
