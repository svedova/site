...
export default () => (
  <Post>
    ...
    quotes and lists: <InlineCode>{`<article>`}</InlineCode>. Although Safari doesn’t require a
    ...
    like <InlineCode>{`<time>`}</InlineCode>, <InlineCode>{`<hr>`}</InlineCode> or other elements which are semantically
    correct within an <InlineCode>{`<article>`}</InlineCode> tag.</P>
