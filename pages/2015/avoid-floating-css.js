...
export default () => (
  <Post>
    ...
    <P>
      Do we still need to use it? Yes, the
      {' '}
      ...
      align items one-after-another.
    </P>

    <P>
      Firstly, there{`'`}s this whole problem that the parent element
      of the aligned items will break down like an old house that
      {`'`}
      s currently
      being demolished by a demolition crane. <i>
        Okay, I{`'`}m digressing a bit, let{`'`}s
        move on.
      </i> So why are we still using it? I mean, pushing an inline
      ...
      making sure they don{`'`}t affect other elements.
    </P>

    <P>
      The answer is: No, we don{`'`}t need to use this
      ...
      {' '}
      are were badly supported by browsers. But
      this time is over, every major browser
      encourages you to use it now.
    </P>

    <P>
      If you{`'`}re also tired of floating elements and want to
      try out something new, continue reading. If not, you{`'`}re completely
      free to leave this page.
    </P>

    <P>
      Alright gentlemen, then start your engines now! Thanks to
      ...
      and also way easier to use.
    </P>

    <H2>Flexbox</H2>

    <P>
      Firstly, there{`'`}s <InlineCode>flex</InlineCode> - I told you about
      ...
      of an item in dependence of the other ones around it.
    </P>

    <Quote author="Chris Coyier - CSS Tricks">
      The main idea behind the flex
      layout is to give the container the ability to alter its items
      {`'`}
      {' '}
      width or height
      in order to best fill the available space...
    </Quote>

    <P>
      The problem with our current box model is that we{`'`}re theoretically
      ...
      is it useful. Sure, there{`'`}s a practical solution for that:
    </P>

    <P>
      Browsers are built to figure things like this out and close
    ...
    <P>
      There are a few more things that I especially like on this
    ...
    <P>
      To start using this method now, just try it{`'`}s easy syntax
...
display: block;`}
    ...
    <P>
      For the first steps, there{`'`}s absolutely no need to add
    ...
    <P>
      ...
      like <InlineCode>justify-content</InlineCode> (which I also used to
      ...
      s
      {' '}
      <Link href="https://developer.mozilla.org/en-US/docs/Web/CSS/flex">
        documentation
      </Link>
      {' '}
      (the
      other related properties are shown on the left there).
    </P>

    <P>
      However, watch out! In the current state of
      art, the <InlineCode>flex</InlineCode> property is supported
      ...
      value (<InlineCode>-webkit-flex</InlineCode>) and not before
      the key (just take a look on the code example up there). All
      other properties can generally be prefixed as
      usual (e.g. <InlineCode>-webkit-justify-content: center</InlineCode>).
    </P>

    <H2>Blocks, but InlineCode</H2>

    <P>
      Yes, that{`'`}s kind of a weird heading but describes
      exactly what I want to talk about now: The
      {' '}
      <InlineCode>inline-block</InlineCode>
      {' '}
      value
      ...
      {' '}
      property. Sure, you
      might already now about it <i>
        (What am I talking
        about, of course you do...)
      </i>, but have you ever thought
      about using it as an alternative for aligning
      columns? I mean, that{`'`}s the use-case for which you
      perhaps needed floats before, right?
    </P>

    <P>
      Okay then. I will now try to be as direct as possible when
    ...
    <P>
      When setting up a portion of clean HTML code, you{`'`}ll probably
    ...
    <P>
      ...
      s clear that it
      {`'`}
      s not the right property for this case).
    </P>

    <P>
      Anyway, don{`'`}t hold yourself back from using it just because it
      ...
      need for and to push the web forward.
    </P>

    <P>
      In contrast to flex, the
      {' '}
      <InlineCode>inline-block</InlineCode>
      {' '}
      value doesn
      ...
      or the middle by using <InlineCode>text-align</InlineCode>. You could also
      use properties like
      {' '}
      ...
      on if they own padding, since it{`'`}s extending the line-height).
    </P>

    <P>
      I{`'`}m sure the most of you already know everything about this
      ...
      they{`'`}re too abstract for you, I guess).
    </P>

    <H2>Conclusion</H2>

    <P>
      My indention behind this article was clearly not to provide
      ...
      you to think about these alternatives, I{`'`}m happy.
    </P>

    <P>
      What you should also remember is that these properties
