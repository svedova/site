...
export default () => (
  <Post>
    ...
    <P>Do we still need to use it? Yes, the <InlineCode>float</InlineCode> property
    ...
    like <InlineCode>flex</InlineCode> are were badly supported by browsers. But
    ...
    <P>Firstly, there{`'`}s <InlineCode>flex</InlineCode> - I told you about
    ...
    the future. And yes, that{`'`}s where <InlineCode>flex</InlineCode> comes in.</P>
    ...
    <Code language="css">{`display: -webkit-flex;
display: block;`}</Code>
    ...
    preview), <InlineCode>align-self</InlineCode> and <InlineCode>order</InlineCode>. If
    ...
    usual (e.g. <InlineCode>-webkit-justify-content: center</InlineCode>).</P>

    <H2>Blocks, but InlineCode</H2>

    <P>Yes, that{`'`}s kind of a weird heading but describes
    exactly what I want to talk about now: The <InlineCode>inline-block</InlineCode> value
    used in combination with the <InlineCode>display</InlineCode> property. Sure, you
    ...
    issue, we need to set <InlineCode>font-size</InlineCode> to <InlineCode>0</InlineCode> or put all items
    ...
    <P>In contrast to flex, the <InlineCode>inline-block</InlineCode> value doesn{`'`}t
    ...
    use properties like <InlineCode>line-height</InlineCode> to equal the heights
    ...
    layouting too, because <InlineCode>grid</InlineCode> isn{`'`}t fully ready for production yet.</P>
  </Post>
)
