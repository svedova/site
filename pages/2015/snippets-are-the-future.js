...
export default () => (
  <Post>
    ...
    <P>
      ...
      which we still need to find a proper fix.
    </P>

    <P>
      Okay, let me explain that before we go on. To me, it
    ...
    <P>
      But I{`'`}m not here to help you with doing so. I{`'`}m here to
      encourage you to think about how we can make the web
      faster while keeping the same level of awesomeness<Ref id="1" />.
    </P>

    <P>
      Great, then let{`'`}s talk about the best compromise I found
      ...
      functions, helpers, etc. and reduce the size of the code that
      {`'`}
      s getting
      presented to the client (which speeds up the
      painting-process within the browser).
    </P>

    <P>
      Cool, huh? Yes, it is.<br />But I know, it{`'`}s far away from perfect.
    </P>

    <P>
      But in my view, it{`'`}s currently the biggest place for our
      fat, effort-shy, fast-clearance-loving asses on the fine
      line between performance and developer-friendliness. Trust me.
    </P>

    <P>
      <i>{`"`}Why isn{`'`}t it perfect?{`"`}</i>
      , you may ask. Easy: While mixing together
    ...
    <FootNotes>
      <Note id="1">
        We shouldn{`'`}t limit our options in this area, developers
        should still be able to build completely
        crazy, dynamic and stunning web-apps.
      </Note>
    </FootNotes>
  </Post>
)
