...
export default () => (
  <Post>
    ...
    <P>
      If you{`'`}re moving around the web once in a while, you certainly
      ...
      easy implementations of their data for developers.
    </P>

    <P>
      The reason behind that is, that iframes have been the
      best way to embed content that can{`'`}t be modified
      by the existing styling of the site which it is
      getting loaded in. — But no longer!
    </P>

    <H2>CSS3 to the Rescue...!</H2>

    <P>
      While digging through Mozilla{`'`}s CSS docs, I was searching
      ...
      s
      {' '}
      <Link href="https://dev.twitter.com/web/embedded-tweets">
        embedded tweets
      </Link>
      , I
      just don
      ...
      t want my plugin
      {`'`}
      s styling to get changed by the existing.
    </P>

    <P>
      So after looking around a bit, I
      found
      {' '}
      <Link href="https://developer.mozilla.org/en-US/docs/Web/CSS/all">
        all
      </Link>
      . I
      ...
      property does, in a nutshell.
    </P>

    <P>
      It helps you to get rid of every decleration. And when I
      say every, I mean every. You might think it resets the styling
      to match the default formatting... Hehe, nope! Every
      property is getting removed.
    </P>
  ...
  all: unset;
    ...
    <P>
      ... doesn{`'`}t just change the heading{`'`}s (let{`'`}s say) font-size
      to 2em (which is the default one in
      HTML5). <i>No!</i> It set{`'`}s the font size to 100%.
    </P>

    <P>
      Now you got it, right? Cool! Then let{`'`}s talk about the
      ...
      able to embed it into their site. But what if you don{`'`}t want that?
    </P>

    <P>
      Instead of providing a fixed iframe for their embedded
      ...
      then put it into a simple div (or maybe into an
      {' '}
      ...
      the embedded Tweet because it would be overridden.
    </P>

    <P>But they could! — If they start using the {`'`}all{`'`}-property now!</P>

    <P>
      Doing so, would remove all styles set by the site itself and
      give Twitter the change to apply their own beautiful tweet-like style.
    </P>

    <H2>Other Use-Cases</H2>

    <P>
      As I already indicated, using the property as a faster alternative
      for iframes is not the only possible case where it can find
      application. Just think about how great it would be to be able
      to code a jQuery plugin which styles can{`'`}t be overriden, for example.
    </P>

    <P>
      As many CSS properties I like to talk of, this one is
      very new<Ref id="2" />. Nevertheless all major browsers (except Safari of
      course) are already able to interpret it. So
      let{`'`}s go, start using it today!
    </P>

    <FootNotes>
      <Note id="1">
        It{`'`}s written in ES6 and will be
        published soon, so stay up-to-date! 😉
      </Note>

      <Note id="2">
        No, seriously. It{`'`}s not even
        on <Link href="http://caniuse.com/#search=all">Can I Use</Link>.
      </Note>
    </FootNotes>
  </Post>
)
