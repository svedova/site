...
export default () => (
  <Post>
    ...
    <P>
      Since I started this blog using Jekyll, I knew this
      ...
      way to manage comments.
    </P>

    <P>
      But over time, I noticed a few things that bugged me
      on this constellation and I never felt really happy
      about the solution I put up for this problem. Thus
      there{`'`}s now time for a change:
    </P>

    <P>
      It{`'`}s not just about the time I{`'`}ve spend with
      ...
      removing comments from my site is way better than keeping them.
    </P>

    <H2>Your Time Is Valued</H2>

    <P>
      Right now, there{`'`}s so much stuff happening on the web. And
      ...
      and read the next one (or even leave my site).
    </P>

    <P>
      After finishing an article, you shouldn{`'`}t be distracted
      ...
      up here to make their work easier and more fun.
    </P>
    ...
    <P>
      Let{`'`}s face it: Disqus is a performance-disaster. I only
      ...
      I will ever get if I keep using this service.
    </P>

    <P>
      I don{`'`}t exactly know why they don{`'`}t care that much about
      ...
      you ask me, loading ca. <b>
        21 uncached scripts, icon-fonts, images
        and stylesheets
      </b> (which all together make around
      253KB) + building a websocket-connection is just too much<Ref id="1" />.
    </P>

    <P>
      Although they{`'`}re loading so much stuff for just one
      tool, they didn{`'`}t even manage to have the commenting input
      resized when the user switches to landscape-mode on mobile devices.
    </P>

    <P>
      Especially as a defender of the open and
      fast web, I simply can{`'`}t affort this amount of
      garbage on my site. If I speak or write about
      something, I want to be able to account for it.
    </P>

    <H2>How Disqus Makes Money</H2>

    <P>
      When I first started using the service, I kept searching
      ...
      {`"`}
      {' '}
      package, because I wanted
      to hide this damn branding at the bottom of the
      iframe (which itself is another argument for removing the system).
    </P>

    <P>
      But sadly, I didn{`'`}t find one. Now, a few months
      ...
      stuff they need to run it? No.
    </P>

    <P>
      I trust them when they say the following in
      their <Link href="http://bit.ly/2jcFuwV">Privacy Policy</Link>:
    </P>

    <Quote>
      We will not rent or sell your Personally Identifiable
      Information to third parties outside Disqus and its controlled
      subsidiaries and affiliates without your consent...
    </Quote>

    <P>
      But that doesn{`'`}t mean they won{`'`}t use the tracking data
      ...
      can{`'`}t allow on my site anymore.
    </P>

    <P>
      And while we{`'`}re on it: Yes, I use Google Analytics to
      ...
      visitors data for creating targeted ads.
    </P>

    <P>
      I also contacted GitHub a while ago and asked them to
      ...
      this something like this up).
    </P>

    <H2>How Does the Future Look Like?</H2>

    <P>
      Firstly, I usually post my articles
      ...
      just write me on Twitter or via e-mail.
    </P>

    <P>
      Using this method, I also make sure that people are not
      ...
      to get the beta by myself, at least their site tells me this).
    </P>

    <P>
      Another great way to make room for your opinion is to simply
      ...
      here (including my own statement).
    </P>

    <HR />

    <P>
      One of the great people who inspired me to do this is John
    ...
    <P>
      I hope you{`'`}re not that disappointed about the fact that you
      ...
      your opinion, only the way how you can interact with me changed, nothing else.
    </P>

    <FootNotes>
      <Note id="1">
        I know that many of them are only loaded after
        the site itself has finished loading completely, but for
        me, the difference that this fact actually makes, is fairly small.
      </Note>
    </FootNotes>
  </Post>
)
