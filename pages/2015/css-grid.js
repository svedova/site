...
export default () => (
  <Post>
    ...
    <P>
      ...
      {' '}
      <Link href="http://bit.ly/2iQvY4g">post</Link>
      {' '}
      on
      ...
      that didn{`'`}t held him back from asking for interest in another one.
    </P>

    <P>
      The reason behind that is for sure that he just likes coding
      ...
      it for the purpose of learning and not for other people. 😉
    </P>

    <P>
      <b>Why?</b> Because the need for those kinds of frameworks
      will soon be gone forever. — Okay, that might be a
      bit confusing for you know, so let{`'`}s start with a quick explanation:
    </P>

    <P>
      I don{`'`}t know how you do it, but since a few years I have
      ...
      or using a mushy JS mixture that does this job.
    </P>

    <P>
      Until now, this was pretty much the best way to create a proper
      website layout (without writing a custom grid of course). Fortunately,
      we{`'`}re now finally getting redeemed from this heavy
      performance load - thanks to <InlineCode>grid</InlineCode>.
    </P>
    ...
    <P>
      ...
      a
      {' '}
      <Link href="http://www.w3.org/TR/css-grid-1/">new CSS method</Link>
      {' '}
      that allows us
      to easily create beautiful web-layouts without a huge
      grid framework. No matter how tiny the budget of the
      client and how short your available time will be,
      {' '}
      <InlineCode>grid</InlineCode>
      {' '}
      provides
      you with many different properties which tell the browser
      how to position an element within the design.
    </P>

    <P>
      And the best thing about that: It seems like the browser
      ...
      right before the body tag closes (after all other elements).
    </P>

    <P>
      It
      {`'`}
      s kind of like using
      {' '}
      <InlineCode>position: absolute</InlineCode>
      {' '}
      but
      without needing to specify the position of
      the element with pixels. <i>
        The grid will figure it
        out by itself.
      </i> — W3C describes this
      behaviour as
      {' '}
      <Link href="http://www.w3.org/TR/css-grid-1/#source-independence">
    ...
    <P>
      Because it{`'`}s a pretty new technology, there{`'`}s currently only
      ...
      makes a bit more sense, right? *laughs*
    </P>

    <P>
      Anyway. If you don{`'`}t use IE (*shame on you*) you{`'`}re also
      able to enable it through the {`"`}experimental Web
      Platform features{`"`} flag
      in <InlineCode>chrome://flags</InlineCode> within Chrome or Opera.
    </P>

    <H2>So Just an Alternative to Flexbox?</H2>

    <P>
      <b>No!</b> — Don{`'`}t start running and yelling {`'`}Fuck you
      flexbox{`'`} around in your office, WE STILL NEED IT!
    </P>

    <P>
      Those two properties are meant to be used for two
      completely different cases. The first -
      {' '}
      <InlineCode>display: grid</InlineCode>
      {' '}
      - can
      ...
      in one row to the same height for example.
    </P>
    ...
    <P>
      So what are you waiting for? Open Chrome, enable that
      experimental flag and try the shit out of
      {' '}
      <InlineCode>grid</InlineCode>
      ! If you get
      ...
      the
      {' '}
      <Link href="http://www.w3.org/TR/css-grid-1/">W3C documentation</Link>
      .
    </P>

    <P>
      There are much more use cases for it to discover and
