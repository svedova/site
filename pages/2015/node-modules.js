...
export default () => (
  <Post>
    ...
    <P>
      First off: I{`'`}m relatively new to node (started developing
      ...
      node currently work and what I would change.
    </P>

    <P>
      I started developing a secret project a while ago
      ...
      why I{`'`}m here now and want to talk about this.
    </P>

    <Image src="/static/essays/2015/node-modules/history-guy.png" />

    <P>
      When talking about open source projects, I{`'`}m usually very
    ...
    <P>
      Anyway. Let{`'`}s talk about the problem: To me, it feels
    ...
    <P>
      Why don{`'`}t we just install all modules globally by default? And
      if a module needs to require it, it will simply be
      loaded from that global directory where all modules live. Some
      advantages this method will entail:
    </P>

    <OL>
      <LI>
        <b>No more duplicated modules:</b> Every time you set up
        ...
        rather than installing it again & again & again.
      </LI>

      <LI>
        <P>
          <b>Goodbye, nesting inception:</b> Okay, let{`'`}s assume we
          ...
          what we want. — So let
          {`'`}
          s install the first module... Ahh, cool! Looks
          ...
          depend on, also have some dependencies.
        </P>

        <P>
          Great! But we could avoid this whole mess by putting all
          those modules on the same level, so that we have a
          great overview rather than a folder in a folder in a folder in a folder.
        </P>
      </LI>

      <LI>
        <b>Alway up-to-date:</b> If some of our apps only need 1.3.0 of a
        ...
        fit the highest dependency). — I hope that{`'`}s understandable.
      </LI>

      <LI>
        <P>
          <b>Faster, more secure and easier on large deployments:</b>
          {' '}
          If there
          {`'`}
          s a
    ...
    <P>
      I{`'`}m not sure but I think we also wouldn{`'`}t be the first ones
      implementing this method. Just think about how Ruby does it
      with Gems. They{`'`}re all in one place. No nesting, no confusion.
    </P>

    <P>
      So what do you think about this? Aren{`'`}t you also tired
      of this never ending story of modules that are nested to
      the moon? If so, then simply write something about your
      own opinion on
      an
      {' '}
      <Link href="https://github.com/joyent/node/issues/25770">issue</Link>
      {' '}
      which I
      {`'`}
      ve created for
    ...
    <P>
      <Edit /> I just read about the cool
      stuff the NPM folks introduced
      with
      {' '}
      <Link href="https://github.com/npm/npm/releases/tag/v3.0.0">3.0.0</Link>
      {' '}
      - the update
    ...
    <Quote>
      ... all of your dependencies, and their dependencies, and
    ...
    <P>
      Okay. Looks like we got this whole nesting-mess solved. I{`'`}m happy
      ...
      probably
      {' '}
      <Link href="https://github.com/npm/npm/issues/2949#issuecomment-11408461">
        never happen
      </Link>
      . Maybe
      I just need to find a way around this.
    </P>

    <HR />

    <P>
      <Edit id="2" /> I wrote another piece regarding
