...
export default () => (
  <Post>
    ...
    <P>
      ...
      {' '}
      <Link href="https://twitter.com/rauchg">rauchg</Link>
      {' '}
      wrote
      me on Slack saying that I should write more asynchronous code because I would otherwise be making
      {' '}
      {'"'}
    ...
    <P>
      ...
      and went completely asynchronous.
    </P>

    <P>
      However, I didn{"'"}t manage to ask him why it{"'"}s like that. We
      were shipping a lot of stuff at that time and I simply forgot about it.
    </P>

    <P>
      Now, nearly a year later, I came across this topic again because native support
      for both keywords
      {' '}
      ...
      {' '}
      and
      I spent a lot of time thinking about how we could take advantage of that at
      {' '}
      <Link href="https://zeit.co">ZEIT</Link>
      . So I collected my thoughts
      and we had a detailed discussion about why everyone should
      {' '}
      <InlineCode>await</InlineCode>
      {' '}
      asynchronous functions, rather than
      using sychronous ones (like <InlineCode>fs.statSync</InlineCode>).
    </P>
    ...
    <P>
      Therefore, I thought making my learning progress public could
      make others - who
      {"'"}
      re in the same position - profit from this knowledge as well. At the same time,
    ...
    <P>
      In the context of computing processes, however, I
      learned that this assumption is not always true:
    </P>

    <P>
      While <b>parallel</b> operations are both started at the
      ...
      run at the same time (suitable for single-core CPUs).<Ref id="1" />
    </P>
...
console.log('Data downloaded')`}
    ...
    <P>
      ...
      calls an arbitrary function which is doing something in a synchronous way.
    </P>
    ...
    <P>
      ...
      gets called, the callback inside
      {' '}
      ...
      {"'"}
      t be run before
      {' '}<InlineCode>loadDataSync()</InlineCode> has returned something.
    </P>
    ...
    <P>
      ...
      {`'`}
      t allow for operations running in parallel
      out of the box.
    </P>
    ...
    <P>
      Or as Panu from
      {' '}
      <Link href="https://bytearcher.com">Byte Archer</Link>
      {' '}
      puts it:
    </P>

    <Quote>
      The event-loop repeatedly takes an event and fires any event handlers listening to that event
    ...
    <P>
      ...
      be enqueued on a certain point in time, but will only start once the thread isn
      {`'`}
      t handling
      any other operation.
    </P>
    ...
    <P>
      As an example, the
      {' '}
      <InlineCode>loadDataSync()</InlineCode>
      {' '}
      function call shown in the
      snippet above might take - let
      {`'`}
      s say -
      {' '}
      <b>five seconds</b>
      {' '}
      to download some data. This would mean
      that the callback of
      {' '}
      <InlineCode>setInterval()</InlineCode>
      {' '}
      will get enqueued after
      {' '}
      <b>1000 milliseconds</b>
      ,
    ...
    <P>
      ...
      immediately after {`"`}the data was downloaded{`"`}:
    </P>
    ...
    <P>
      To solve this problem, we need to make the operation
...
console.log('Data downloaded')`}
    ...
    <P>
      To make this work, you would also have to rewrite your sychronous function into an asynchronous
      one (either a <InlineCode>Promise</InlineCode> or a function
      prefixed with <InlineCode>async</InlineCode>).
    </P>

    <P>
      To make my point clear, I came up with an arrow
      function that simulates the case of
      {' '}
      <InlineCode>loadData()</InlineCode>
      {' '}
      taking 5000 milliseconds
      to finish:
    </P>
  ...
  setTimeout(resolve, 5000)
    ...
    <P>
      As you can see, even though the function is now acting asynchronously, the interval output never
      shows up after exact 1000 milliseconds. It
      {`'`}
    ...
    <P>
      ...
      <InlineCode>await</InlineCode>
      {' '}
      keyword. As indicated by the name, it could
    ...
    <P>
      Instead, actions like fetching raw data and such are processed
      directly by the
      {' '}
    ...
    <P>
      In turn, part of our code is still running concurrently. Both the processing
    ...
    <P>
      A process can contain multiple threads. Each of these threads can
...
if (cluster.isMaster) {
  ...
  console.log('Data downloaded')
    ...
    <P>
      I hope this post helped you to sharpen your mindset for
      being able to choose the best direction when it comes to achieving
      the maximum of performance for your future projects.
    </P>
    ...
    <P>
      Big 🤗 to
      {' '}
      <Link href="https://twitter.com/OVanhoja">Olli</Link>
      {' '}
      and
      {' '}
      <Link href="https://twitter.com/rauchg">Guillermo</Link>
      {' '}
      for
      ...
      essay) and
      {' '}
      <Link href="https://twitter.com/nVoidRayy">Matthias</Link>
      {' '}
